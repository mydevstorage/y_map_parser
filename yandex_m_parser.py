...
logger.add(f'{DATA_FOLDER}/data/logger.log',
...
def create_file_for_links():
    with open(f"{DATA_FOLDER}/data/links.txt", 'w') as file:
        file.write('')
...
def get_all_links(city, request):
    try:
        ...
        logger.info(f'Для города <{city}>, ссылки собраны')
    except Exception as ex:
        logger.error(ex)
    ...
    with open(f"{DATA_FOLDER}/data/links.txt", "r") as file:
        for link in file:
            try:
                ...
                print(f'Обработано {partner_id} компаний')
            except Exception as ex:
                logger.error(ex)
    # print(f'Город {city} обработан, осталось {171 - dict_city[city]} городов')
...
def main():
    try:
        ...
        city, request = ['Город'], ''               # ask_city_and_request()
        for current_city in city:
            # get_all_links(city, request)
            process_all_links(current_city)
        x2 = time.time() - x1
        print(int(x2))
    except Exception as ex:
        print(ex)
